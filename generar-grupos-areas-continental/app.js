  ...
  try {
    ...
    try {
      empleadosRes = await reqRead.query(
        `SELECT Nomina, Nombre, FechaAlta, CentroCoste, Posicion, UnidadOrganizativa, Rol FROM [${schema}].[Empleados]`
      );
    } catch (readErr) {
    ...
    // 4. Insertar Areas únicas
    const areaIdMap = new Map(); // UnidadOrganizativa -> AreaId
    const areasVistas = new Set();
    for (const row of empleados) {
      const key = row.UnidadOrganizativa;
      if (!key || areasVistas.has(key)) continue;
      areasVistas.add(key);

      const reqArea = new sql.Request(transaction);
      reqArea.input('uo', sql.VarChar, row.UnidadOrganizativa);
      reqArea.input('ng', sql.VarChar, row.NombreGeneral);
      const areaInsert = await reqArea.query(
        `INSERT INTO [${schema}].[Areas] (UnidadOrganizativaSap, NombreGeneral) VALUES (@uo, @ng); SELECT SCOPE_IDENTITY() AS id;`
      );
      const areaId = areaInsert.recordset && areaInsert.recordset[0] && areaInsert.recordset[0].id;
      areaIdMap.set(row.UnidadOrganizativa, areaId);
    }

    // 5. Insertar Grupos únicos por (UnidadOrganizativa, Rol)
    const grupoIdMap = new Map(); // `${uo}|${rol}` -> GrupoId
